...
void Widget::UpdateModel()
{
    ...
    if(button.GetState() != SelectButton::State::Setting) // nếu đang cài đặt thì ngừng chơi
    {
        if(field.GetState() == MemeField::State::Memeing && field.GetRect().Contains(mousePos)) // Đang chơi và tọa độ chuột trong cánh đồng
        {
            ...
            {
                // Gắn cờ khi nhấn chuột phải
                field.OnFlagClick(mousePos);
            }
            else if (type == Mouse::Type::LPress) // Nếu loại sự kiện == nhấn chuột trái.
            {
                // Tiết lộ khi nhấn chuột trái
                field.OnRevealClick(mousePos); // Tọa độ chuột
            }
...
void Widget::mousePressEvent(QMouseEvent *event) // nhận sự kiện giữ chuột
{
    ...
    // QSinglePointEvent->Qt::MouseButton(Enum)    // Nút bên trái được nhấn hoặc một sự kiện đề cập đến nút bên trái.
    if (event->button() == Qt::LeftButton) // Nếu enum sự kiện chuột == Chuột trái
    {
    ...
    // Làm mới đồ họa mỗi khi chuột được nhấn
    repaint(); // gọi hàm paintEvent để vẽ lại widget // lưu ý: dùng trong hàm paintEvent sẽ gây đệ quy, thay vào đó dùng
}
